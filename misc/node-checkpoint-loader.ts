...
export class NodeCheckpointLoader {
  ...
  constructor(private checkpointFilePath: string) {
    this.checkpointManifest = require(path.resolve(this.checkpointFilePath, 'manifest.json'));
  }
  ...
  getVariable(varName: string): Promise<dl.Tensor> {
    ...
    const filePath = path.resolve(this.checkpointFilePath, fileName);
    return new Promise<dl.Tensor>((resolve, reject) => fs.readFile(filePath, (err, buffer) => {
      if (err) {
        return reject(err);
      }
      const values = new Float32Array(buffer.buffer);
      resolve(dl.Tensor.make(this.checkpointManifest[varName].shape, { values }));
    }));
  }
}